...
- [ ] **S7 検証(strip-then-restore の前提。最初にやる)**: 境界の `.transaction {}` フックで `transaction.animation` に非 nil を代入してアニメーションを復元できることを spy テストで確認する(Phase 0 では未検証)。不成立なら strip-then-restore を中止し、代替案を添えてメンテナに報告して停止
- [ ] `AnimationScope`(値駆動 + proxy 駆動)、`.animationBarrier()`、刻印(§4.1 strip-then-restore セマンティクス)
...
- [ ] README: ヒーロー GIF はプレースホルダで良い(録画は人間がやる)。ただし撮影対象となるデモ画面はサンプルアプリに用意すること
