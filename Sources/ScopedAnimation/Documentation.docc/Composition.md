...
動かしたい領域ごとに、アニメーションを与える所有者と、外からのアニメーションを除去する境界を決めます。

| 領域の関係 | 構成 |
| --- | --- |
| 別々の表示レイヤー | 兄弟スコープ |
| 同じサブツリーに複数の条件 | 1 スコープの複数トリガー |
| 子孫に独立した所有者が必要 | 入れ子のスコープ |
| 外からのアニメーションだけを除去 | バリア |
...
親と子が同時に変化した場合も、子孫のスコープは自身のアニメーションと所有者スタンプを適用します。
